...
export function Question11({ index = 0, isActive = false }: { index?: number; isActive?: boolean }) {
  ...
  return (
    ...
    >
      ...
      <PlainText>
        Arrow functions are not just syntactic sugar - they have <Bold>different behavior</Bold> regarding the <InlineCode>this</InlineCode> keyword, making them particularly useful for certain scenarios.
      </PlainText>
...
const createPerson = (name, age) => ({ name, age });`}
        ...
        <CardComponent variant="success" title="🎯 When to Use Arrow Functions (Beginner)">
          <UnorderedList
            items={[
              <>Simple transformations with array methods like <InlineCode>map()</InlineCode>, <InlineCode>filter()</InlineCode></>,
        ...
        <CardComponent variant="warning" title="⚠️ Common Beginner Mistakes">
          <UnorderedList
            items={[
              <>Forgetting parentheses for multiple parameters: <InlineCode>{`(a, b) =>`}</InlineCode></>,
              <>Forgetting parentheses for object return: <InlineCode>{`() => ({ name })`}</InlineCode></>,
              <>Using arrow functions when you need the original <InlineCode>this</InlineCode> context</>,
            ]}
          />
        </CardComponent>

        <Note type="success" icon="✅">
          <Bold>Practice Exercise:</Bold> Try converting this traditional function to an arrow function:
          <CodeComponent
            code={`// Convert this:
...
button.addEventListener('click', () => {
  console.log(this); // window
        ...
        <CardComponent variant="info" title="📊 Arrow vs Traditional Comparison">
          <TableComponent
        ...
        <Note type="info" icon="💡">
          <Bold>Pro Tip:</Bold> In React class components, arrow functions are commonly used for event handlers to avoid manually binding <InlineCode>this</InlineCode> in the constructor.
        </Note>
      </LevelContent>
      ...
      <LevelContent level="expert" currentLevel={level}>
        ...
        <PlainText>
          Deep dive into advanced arrow function patterns, performance considerations, and expert-level techniques:
        </PlainText>
...
console.log(safeDivide(10, 0)); // { success: false, error: 'Cannot divide by zero' }`}
        ...
        <CardComponent variant="info" title="Performance Tips">
          <UnorderedList
            items={[
              ...
              <>Use <InlineCode>useCallback</InlineCode> with arrow functions in React to prevent re-renders</>,
              <>Avoid arrow functions in hot code paths if performance is critical</>,
              <>Arrow functions create new function instances - use <InlineCode>useCallback</InlineCode> or <InlineCode>useMemo</InlineCode> for memoization</>,
            ]}
          />
        </CardComponent>
        ...
        <CardComponent variant="default" title="Pro Insights">
          <UnorderedList
            items={[
              <>Combine arrow functions with <Bold>destructuring</Bold> for cleaner parameter handling</>,
              <>Use arrow functions in <Bold>pipelines</Bold> for data transformation</>,
              ...
              <>Implement <Bold>currying</Bold> and <Bold>partial application</Bold> for reusable functions</>,
              <>Use <Bold>functional composition</Bold> for building complex behaviors</>,
...
const arrowFn2 = () => {
  console.log(new.target); // SyntaxError
        ...
        <HLText type="info">
          💡 <Bold>Expert Insight:</Bold> Arrow functions are a powerful tool for functional programming. Understanding their behavior with <InlineCode>this</InlineCode>, <InlineCode>arguments</InlineCode>, and <InlineCode>new.target</InlineCode> is essential for writing advanced JavaScript applications. They enable a more <Bold>functional</Bold> style of programming that promotes <Bold>immutability</Bold> and <Bold>composability</Bold>.
        </HLText>

        <Note type="warning" icon="⚠️">
          <Bold>Expert Warning:</Bold> While arrow functions are powerful, they're not a silver bullet. Overusing them can lead to code that's harder to understand, especially when debugging <InlineCode>this</InlineCode> issues. Use them where they make sense, but don't hesitate to use traditional functions when appropriate.
        </Note>
      </LevelContent>
      ...
      <CardComponent variant="info" title="🎯 Key Takeaways">
        <UnorderedList
          items={[
            ...
            <>Advanced patterns include <Bold>currying</Bold>, <Bold>composition</Bold>, and <Bold>memoization</Bold></>,
          ]}
        />
      </CardComponent>
      ...
      <Note type="success" icon="🎉">
        <Bold>Remember:</Bold> Master arrow functions to write cleaner, more modern JavaScript code! Start with the basics, understand <InlineCode>this</InlineCode> binding, and then explore advanced functional patterns.
      </Note>
    </QuestionWrapper>
  );
}